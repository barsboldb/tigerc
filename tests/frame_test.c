...
int test_frame_new_no_params() {
  frame_t *f = frame_new("f", NULL, 0, 0);
  ASSERT(f != NULL);
...
int test_frame_new_all_escaped() {
  int escapes[] = { 1, 1, 1 };
  frame_t *f = frame_new("f", escapes, 3, 0);
  ASSERT(f != NULL);
...
int test_frame_new_mixed_escapes() {
  int escapes[] = { 0, 1, 0, 1 };
  frame_t *f = frame_new("f", escapes, 4, 0);
  ASSERT(f != NULL);
...
int test_frame_alloc_local_escaped() {
  int escapes[] = { 1 };
  frame_t *f = frame_new("f", escapes, 1, 0);
...
int test_frame_alloc_local_after_escaped_params() {
  int escapes[] = { 1, 1 };
  frame_t *f = frame_new("f", escapes, 2, 0);
